...
case class Dice private (private val rep: Distribution[Int]) {
  ...
  /**
    ...
    * the results.
    */
  def repeat(n: Dice): Dice =
    for {
      v <- n
      r <- Range(0, v).map(_ => this).foldLeft(this)(_ + _)
    } yield r
  ...
  /**
    ...
    */
  def <=(d: Dice): Distribution[Boolean] = rep.productBy(d.rep)(_ <= _)
